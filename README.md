...
2. **Heartbeat (mandatory start/end pair around each chat request)** — every chat request must be bracketed by a matching pair of heartbeats. At the **start of each chat request** (a new user prompt), the agent calls `heartbeat` with `phase: "start"` so the dashboard shows the chat as **running** (which suppresses the idle/TTL countdown while it works). Immediately before its final response text — after **all** the turns and tool calls needed to respond — it calls `heartbeat` with `phase: "end"` so the idle/TTL countdown resumes. It must never send a final response without a matching `phase: "end"`: a missing `end` leaves the chat **falsely showing as running** when it is actually idle. This applies even when a request ends early — if the agent asks a clarifying question, hands off, or stops, it still sends `phase: "end"` first. It may also send plain `heartbeat` pings (default `phase: "progress"`) after meaningful steps in between. This feeds the external dashboard/notifier; it is **not** a checkpoint trigger.
...
3. At the **end** of the chat request, call `heartbeat` with `phase: "end"` first, immediately before the final response text — this closes the running turn so the idle/TTL countdown resumes. This is mandatory and pairs with the `phase: "start"` in step 1: never send the final response without it, even when the request ends early (a clarifying question, handoff, or stop), or the chat will falsely keep showing as running.
