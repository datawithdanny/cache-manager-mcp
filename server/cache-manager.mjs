...
const SERVER_INSTRUCTIONS = [
  ...
  "2. HEARTBEAT — every chat request must be bracketed by a matching pair of heartbeats; this is mandatory, not optional. At the very start of each chat request (a new user prompt), before other work, call heartbeat with phase:'start' so the dashboard shows the chat 'running'. Immediately before your final response text — after ALL the turns/tool calls needed to respond — call heartbeat with phase:'end' so the idle/TTL countdown resumes. Never send your final response without a matching phase:'end': a missing 'end' leaves the chat falsely showing 'running' when it is actually idle. This applies even when a request ends early — if you ask a clarifying question, hand off, or stop, still send phase:'end' first. Optionally send plain heartbeat pings (phase:'progress') after meaningful steps in between. This feeds the external dashboard; it is required for the dashboard, not a checkpoint trigger.",
